...
// Fetch pengajuan barang data
$pengajuanData = getPengajuanBarang();

// Handle search
$search = isset($_POST['search']) ? $_POST['search'] : '';
if ($search) {
    $pengajuanData = searchPengajuanBarang($search);
}
...
if (isset($_GET['action']) && isset($_GET['id'])) {
    ...
    if ($action == 'terima') {
        ...
        $stmt = $conn->prepare("UPDATE pengiriman_barang SET status_pengiriman = 'diterima' WHERE id_pengiriman = ?");
        $stmt->execute([$id_pengiriman]);
    } elseif ($action == 'tolak') {
        // Update status ke 'ditolak'
        $stmt = $conn->prepare("UPDATE pengiriman_barang SET status_pengiriman = 'ditolak' WHERE id_pengiriman = ?");
        $stmt->execute([$id_pengiriman]);
    }
  
    header('Location: pengajuan-barang.php');
    exit;
  }
?>
<!DOCTYPE html>
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4>Pengajuan Pengiriman Barang</h4>
                  <div class="card-header-form">
                    <form method="POST" action="pengajuan-barang.php">
                    ...
                    <table id="dataTable" class="table table-md table-hover">
                        ...
                        <tbody>
                            <?php if (count($pengajuanData) > 0): ?>
                                <?php foreach ($pengajuanData as $index => $pengiriman): ?>
                                <tr>
                                    <td><?= $index + 1 ?></td>
                                    ...
                                    <td>
                                      <div class="buttons">
                                        <a href="pengajuan-barang.php?action=terima&id=<?= $pengiriman['id_pengiriman'] ?>" class="btn btn-icon btn-success" title="Terima">
                                          <i class="fas fa-check"></i>
                                        </a>
                                        <a href="pengajuan-barang.php?action=tolak&id=<?= $pengiriman['id_pengiriman'] ?>" class="btn btn-icon btn-danger" title="Tolak">
                            ...
                            <?php else: ?>
                                <tr>
                                    <td colspan="12" class="text-center">
                                        Tidak ada data pengajuan pengiriman barang
                                    </td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
