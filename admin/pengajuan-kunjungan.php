...
include 'functions/pengajuan-helper.php';
...
// Fetch pengajuan kunjungan data
$pengajuanData = getPengajuanKunjungan();

// Handle search
$search = isset($_POST['search']) ? $_POST['search'] : '';
if ($search) {
    $pengajuanData = searchPengajuanKunjungan($search);
}
...
if (isset($_GET['action']) && isset($_GET['id'])) {
  ...
  if ($action == 'terima') {
      ...
      $stmt = $conn->prepare("UPDATE kunjungan SET status_kunjungan = 'diterima' WHERE id_kunjungan = ?");
      $stmt->execute([$id_kunjungan]);
  } elseif ($action == 'tolak') {
      // Update status ke 'ditolak'
      $stmt = $conn->prepare("UPDATE kunjungan SET status_kunjungan = 'ditolak' WHERE id_kunjungan = ?");
      $stmt->execute([$id_kunjungan]);
  }

  header('Location: pengajuan-kunjungan.php');
  exit;
}
?>
<!DOCTYPE html>
            ...
            <div class="col-12">
              <div class="card">
                <div class="card-header">
                  <h4>Pengajuan Kunjungan</h4>
                  <div class="card-header-form">
                    <form method="POST" action="pengajuan-kunjungan.php">
                ...
                <div class="card-body p-0">
                  <div class="table-responsive">
                    <table id="dataTable" class="table table-md table-hover text-center">
                        ...
                        <tbody>
                        <?php if (count($pengajuanData) > 0): ?>
                        <?php foreach ($pengajuanData as $index => $kunjungan): ?>
                                <tr>
                                    <td><?= $index + 1 ?></td>
                                    ...
                                    <td>
                                      <div class="buttons">
                                        <a href="pengajuan-kunjungan.php?action=terima&id=<?= $kunjungan['id_kunjungan'] ?>" class="btn btn-icon btn-success" title="Terima">
                                          <i class="fas fa-check"></i>
                                        </a>
                                        <a href="pengajuan-kunjungan.php?action=tolak&id=<?= $kunjungan['id_kunjungan'] ?>" class="btn btn-icon btn-danger" title="Tolak">
                            ...
                            <?php else: ?>
                          <tr>
                              <td colspan="12" class="text-center">Tidak ada data pengajuan kunjungan</td>
                          </tr>
                      <?php endif; ?>
                        </tbody>
                    </table>
