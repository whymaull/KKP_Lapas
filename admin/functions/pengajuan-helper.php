...
// Function to get all pengajuan kunjungan
function getPengajuanKunjungan() {
    global $conn;
    $stmt = $conn->prepare("SELECT kunjungan.*, 
                            ...
                            WHERE kunjungan.status_kunjungan = 'menunggu'"); 
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Function to get all pengajuan barang
function getPengajuanBarang() {
    ...
    $stmt = $conn->prepare("SELECT pengiriman_barang.*, 
                            ...
                            WHERE pengiriman_barang.status_pengiriman = 'menunggu'"); 
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

// Function to search pengajuan kunjungan based on search input
function searchPengajuanKunjungan($search) {
    global $conn;
    $stmt = $conn->prepare("SELECT kunjungan.*, 
                                   users.nama_lengkap AS nama_pengunjung, 
                                   users.email,
...
// Function to search pengajuan pengiriman_barang based on search input
function searchPengajuanBarang($search) {
    global $conn;
    $stmt = $conn->prepare("SELECT pengiriman_barang.*, 
                                   users.nama_lengkap AS nama_pengirim, 
                                   users.email,
